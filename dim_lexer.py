...
class Lexer:
    ...
    def tokenize(self) -> List[Token]:
        ...
        self.tokens.append(
            Token(
                TokenType.EOF,
                None,
                Span(self.filename, self.line, self.col, self.line, self.col),
            )
        )
        return self.tokens
    ...
    def _process_line(self, raw_line: str):
        ...
        sub = _LineLexer(
            raw_line.lstrip(), self.filename, self.line, indent + 1, self.diag
        )
...
class _LineLexer:
    ...
    def _span(self, start_col: int) -> Span:
        return Span(self.filename, self.line_no, start_col, self.line_no, self._col())
    ...
    def lex(self) -> List[Token]:
        ...
        while self.pos < len(self.text):
            ...
            # ── Numbers ───────────────────────────────────────────────────────
            if ch.isdigit() or (ch == "-" and self._peek(1).isdigit() and not tokens):
            ...
            # Unknown character
            self.diag.error(
                "E0001", f"Unknown character: {ch!r}", self._span(start_col)
            )
            self._advance()
    ...
    def _lex_number(self, start_col: int) -> Token:
        ...
        try:
            ...
            self.diag.error(
                "E0003", f"Invalid number literal: {raw!r}", self._span(start_col)
            )
            val = 0
