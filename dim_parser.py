...
class Parser:
    def __init__(
        self, tokens: List[Token], source: str = "", filename: str = "<stdin>"
    ):
        self.tokens = tokens
        self.pos = 0
        self.source = source
    ...
    def _expect(self, kind: TokenType, msg: str = "") -> Token:
        ...
        if tok.kind != kind:
            self.diag.error(
                "E0010", msg or f"Expected {kind.name}, found `{tok.value}`", tok.span
            )
    ...
    def _expect_kw(self, word: str) -> Token:
        ...
        if not (tok.kind == TokenType.KEYWORD and tok.value == word):
            self.diag.error(
                "E0010", f"Expected keyword `{word}`, found `{tok.value}`", tok.span
            )
    ...
    def _end_span(self, start: Span) -> Span:
        return start.merge(
            self._peek(-1 if self.pos > 0 else 0).span if self.pos > 0 else start
        )
    ...
    def _parse_top_level(self) -> Optional[Statement]:
        ...
        if self._check_kw("enum"):
            return self._parse_enum()
        if self._check_kw("trait"):
            return self._parse_trait()
        if self._check_kw("impl"):
            return self._parse_impl()
        if self._check_kw("prompt"):
            return self._parse_prompt()
        if self._check_kw("actor"):
            ...
            if not self._check(TokenType.RBRACKET):
                ...
                if self._check(TokenType.LPAREN):
                    ...
                    if self._check(TokenType.STRING):
                        perms[-1] += (
                            "(" + (self._expect(TokenType.STRING).value or "") + ")"
                        )
                ...
                while self._match(TokenType.COMMA):
                    if self._check(TokenType.IDENTIFIER):
                        perm = self._expect(TokenType.IDENTIFIER).value
                        if self._check(TokenType.LPAREN):
                            self._advance()
                            if self._check(TokenType.STRING):
                                perm += (
                        ...
                        perms.append(self._expect(TokenType.STRING).value or "")
            self._expect(TokenType.RBRACKET)
        self._expect(TokenType.RPAREN)
        fn = self._parse_function()
        fn.tool = ToolDecorator("tool", perms, span=self._end_span(start))
        return fn
    ...
    def _parse_prompt(self) -> PromptDef:
        ...
        while not self._check(TokenType.DEDENT) and not self._check(TokenType.EOF):
            ...
            if self._check_kw("role"):
                ...
                roles.append(
                    PromptRole(role_name, content, span=self._end_span(r_start))
                )
        ...
        self._match(TokenType.DEDENT)
        return PromptDef(
            name, base, roles, None, output_type, span=self._end_span(start)
        )
    ...
    def _parse_actor(self) -> ActorDef:
        ...
        while not self._check(TokenType.DEDENT) and not self._check(TokenType.EOF):
            ...
            if self._check_kw("receive"):
                ...
                handlers.append(
                    FunctionDef(
                        msg_name, params, ret, body, span=self._end_span(fn_tok_start)
                    )
                )
            else:
                fname = self._expect(TokenType.IDENTIFIER).value
